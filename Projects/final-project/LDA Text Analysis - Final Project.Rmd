...
MalaysiaLDAdataset <- tokenizedLDADataset_stopped %>%
  filter(Primary_Country == "Malaysia")

dtmMALAYSIA <- MalaysiaLDAdataset %>%
  count(Primary_Country, word) %>%
  cast_dtm(Primary_Country, word, n)

#Specifying the number of topics we want for our country analysis

analysisLDAMALAYSIA <- LDA(dtmMALAYSIA, 3, method = "Gibbs", control = list(seed = 123))

topicsMalaysia <- tidy(analysisLDAMALAYSIA, matrix = "beta")
head(topics)
