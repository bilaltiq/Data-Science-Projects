...
Solution: No, regression does not seem appropriate in this scenario as it does not show the variations well enough on the y axis.
...
Solution: Colors appear which have a similar shade to distinguish between the countries.
...
Solution: A qualitative color scheme makes sense for this plot because the divergent pallete has a white color which is not really visible.
...
Solution: Lets use Purple, Turqoise, Blue
...
Solution: The points are all joined togeteher with lines
...
Solution: Each point is treated as a sort of "step". Not sure how to describe it
...
Solution: For each specific point we can view the data for that by hovering over it
...
Solution: No, the interactivity won't remain when we compile to a pdf because they are static formats and not dynamic and interactable.
